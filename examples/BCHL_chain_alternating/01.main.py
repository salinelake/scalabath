...
DEFAULT_BOSON_DIMS = np.asarray([5, 5, 5, 5, 5, 5], dtype=int)
...
def main() -> None:
    ...
    for mode_index, damping_rate in enumerate(gamma):
        jump_operator = np.sqrt(2 * damping_rate) * bath_basis[mode_index].annihilation
    ...
    while steps_done < nsteps:
        ...
        ## we don't normalize here because the simulator normalizes each step
