...
def test_compose_matches_numpy_kron_for_batched_operators() -> None:
    X = jnp.asarray(
        [
            [[0, 1], [1, 0]],
            [[0, -1j], [1j, 0]],
    ...
    result = compose([X, Z])
    expected = np.stack([np.kron(np.asarray(X[idx]), np.asarray(Z[idx])) for idx in range(2)])

    assert result.shape == (2, 4, 4)
    np.testing.assert_allclose(np.asarray(result), expected)


def test_compose_rejects_mismatched_batched_operator_sizes() -> None:
    one_batch = jnp.ones((1, 2, 2), dtype=jnp.complex128)
    two_batches = jnp.ones((2, 2, 2), dtype=jnp.complex128)

    with pytest.raises(ValueError, match="same batch size"):
        compose([one_batch, two_batches])
